        ...
        <v-card>
          ...
          <v-dialog v-model="dialog" max-width="500px">
            ...
            <v-card>
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="blue darken-1" text @click="showEditDialog()">Cancel
                </v-btn>
...
export default {
  data() {
    return {
      headers: [
        {text: "Date operation", value: "date_operation", width: "100"},
        {text: "Qte controlée", value: "qte_controlée", width: "100"},
        {text: "Qte ok", value: "qte_ok", width: "100"},
        {text: "Qte Not ok", value: "qte_notOk", width: "100"},
        {text: "Action", value: "actions", sortable: false, width: "50"},
        {text: "Rapport", value: "operations", width: "150"},
  ...
  methods: {
    ...
    loadItems() {
      ...
      const urlll = "http://139.99.238.74:8000/api/" + this.$route.params.id + "/datepointage/getrap";
      axios
          .get(urlll, {
            headers: {Authorization: "Bearer " + apiToken}
          })
          .then(response => {
            this.items = response.data.rapport.map(item => {
              console.log(item);
              return {
                //id: item.id,
                ...item,
    ...
    saveItem(item) {
      ...
      let method = "post";
      let url = `http://139.99.238.74:8000/api/` + this.$route.params.id + "/Operation";
      const id = item.id;
      ...
      if (id) {
        ...
        method = "put";
        url = `http://139.99.238.74:8000/api/` + this.$route.params.id + `/Operation/${id}`;
    ...
    deleteItem(item) {
      ...
      if (confirm("Are you sure you want to delete this?")) {
        const url = `http://139.99.238.74:8000/api/` + this.$route.params.id + `/Operation/${id}`;
        const apiToken = this.$store.getters.getToken;
        axios.delete(url,
            {
              headers: {
